...
export default function ContratosTable({ contratos, selectedRows, onSelectRow, onSelectAll, onEdit, onDelete }: Props) {
  const todosSeleccionados = contratos.length > 0 && contratos.every((c) => selectedRows.includes(c.id_contrato));

  if (contratos.length === 0) {
    return <div className="py-16 text-center text-gray-400 text-sm">No hay contratos que mostrar.</div>;
  }
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-50">
          {contratos.map((c) => (
            <tr key={c.id_contrato} className={`hover:bg-gray-50 transition-colors ${selectedRows.includes(c.id_contrato) ? "bg-amber-50/40" : ""}`}>
              ...
              <td className="py-2.5 px-3">
                <div className="flex items-center justify-center gap-1.5">
                  ...
                  <button onClick={() => onDelete(c.id_contrato)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Eliminar">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
